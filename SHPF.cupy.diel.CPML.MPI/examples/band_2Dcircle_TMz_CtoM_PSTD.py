...
TF = space2D.Basic2D('TM', (Nx, Ny), (dx, dy), dt, Tsteps, np.complex64, np.complex64, method='PSTD', engine='cupy')
...
if lamby == 0: 
    ...
    ky = 2*np.pi / lamby
    theta = np.arctan(lambx/lamby)

wvlen = lambx*np.cos(theta)
...
filename = "CtoM/lamby{:05d}" .format(int(round(lamby/nm)))
