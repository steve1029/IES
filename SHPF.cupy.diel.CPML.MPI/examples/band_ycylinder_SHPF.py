...
Lx, Ly, Lz = 574*nm, 574/8*nm, 574*nm
Nx, Ny, Nz = 256, 32, 256
...
setter1 = source.Setter(TF, (440*nm, 0, 140*nm), (440*nm+dx, Ly, 140*nm+dz), mmt)
setter2 = source.Setter(TF, (100*nm, 0, 200*nm), (100*nm+dx, Ly, 200*nm+dz), mmt)
setter3 = source.Setter(TF, (200*nm, 0, 300*nm), (200*nm+dx, Ly, 300*nm+dz), mmt)
setter4 = source.Setter(TF, (300*nm, 0, 400*nm), (300*nm+dx, Ly, 400*nm+dz), mmt)
...
loc1 = (120*nm, Ly/2, 100*nm)
loc2 = (450*nm, Ly/2, 300*nm)
loc3 = ( 90*nm, Ly/2, 270*nm)
loc4 = (300*nm, Ly/2, 480*nm)
loc5 = (250*nm, Ly/2, 250*nm)
...
for tstep in range(Tsteps):
    ...
    if tstep % plot_per == 0:
        ...
        #TFgraphtool.plot2D3D(Ex, tstep, yidx=TF.Nyc, colordeep=1e-2., stride=3, zlim=1e-2)
        TFgraphtool.plot2D3D(Ey, tstep, yidx=TF.Nyc, colordeep=1e-2, stride=3, zlim=1e-2)
        #TFgraphtool.plot2D3D(Ez, tstep, xidx=TF.Nxc, colordeep=.1, stride=1, zlim=.1)
        #TFgraphtool.plot2D3D(Hx, tstep, xidx=TF.Nxc, colordeep=.1, stride=1, zlim=.1)
        #TFgraphtool.plot2D3D(Hy, tstep, xidx=TF.Nxc, colordeep=.1, stride=1, zlim=.1)
        #TFgraphtool.plot2D3D(Hz, tstep, xidx=TF.Nxc, colordeep=.1, stride=1, zlim=.1)
...
if TF.MPIrank == 0: recorder.Recorder(TF, start_time, '../record/')
