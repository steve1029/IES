...
void get_deriv_y_E_FML(
	...
	#pragma omp parallel for \
		shared(\
			myNx, Ny, Nz, \
			diffyEx_re, diffyEx_T, \
			diffyEz_re, diffyEz_T, \
		) \
		private( \
			i, j, k, myidx, myidx_T \
		)
		...
		for(j=0; j < Ny; j++){
			for(k=0; k < Nz; k++){
				...
				if(i < (myNx-1)){
					...
					else printf("Here's the wrong part!\n");
				}
...
void get_deriv_x_E_FM0(
	...
	#pragma omp parallel for \
		shared(\
			...
			diffxEz_re, Ez_re, \
			dx, \
			recvEylast_re, recvEzlast_re \
		) \
		private( \
			i, j, k, myidx, i_myidx, myidx_0 \
		)
	for(i=0; i < myNx; i++){
		for(j=0; j < Ny; j++){
			for(k=0; k < Nz; k++){
...
void updateH(										\
	...
	#pragma omp parallel for \
		shared(\
			...
			dt, \
			Hx_re, Hy_re, Hz_re, \
			diffyEz_re, diffzEy_re, \
			diffzEx_re, diffxEz_re, \
			diffxEy_re, diffyEx_re \
		) \
		private( \
			i, j, k, myidx, \
			CHx1, CHy1, CHz1, \
...
void updateE(
	...
	#pragma omp parallel for \
		shared( \
			...
			dt, \
			Ex_re, Ey_re, Ez_re, \
			diffyHz_re, diffzHy_re, \
			diffzHx_re, diffxHz_re, \
			diffxHy_re, diffyHx_re \
		) \
		private( \
			i, j, k, myidx, \
			CEx1, CEy1, CEz1, \
