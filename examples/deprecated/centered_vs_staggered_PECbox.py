...
savedir = f'../graph/cg_vs_sg/{method}/'
...
f"""
    Structures:
        ...
        {Box.name}:
            global x location: {Box.gxloc}
             local x location: {Box.lxloc}
            global y location: {Box.ysrt}, {Box.yend}
            global z location: {Box.zsrt}, {Box.zend}
"""
...
for tstep in range(Tsteps):
    ...
    if tstep % plot_per == 0:
        ...
        TFgraphtool.plot2D3D(Ey, tstep, yidx=TF.Nyc, colordeep=2, stride=1, zlim=2, savenpy=True)
